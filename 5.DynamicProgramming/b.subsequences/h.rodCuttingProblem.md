...
- let'e take an example if we have 5 length target and we have price list of length 5 then we can cut in 1,2,3,4,5 , these many lengths , so we can think of it as, at each indexes as collecting rod of (index+1)length at each index
